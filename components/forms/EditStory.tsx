...
export const EditStory = ({ onSubmit, onClose, story, editable }: Props) => {
    ...
    const editDescription = async () => {
        if (editing) {
            await axios.put(`${LOCAL_HOST}/story/${story._id}`);
            isEditing(false)
        } else {
            isEditing(true)
        }
    ...
    <View style={styles.container}>
        ...
        {editing ? <CustomInput multiline value={description} onChangeText={setDescription}>{story.description}</CustomInput> : <Text>{story.description}</Text>}
